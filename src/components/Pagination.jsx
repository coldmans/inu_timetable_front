    ...
    <nav aria-label="검색 결과 페이지 이동" className="flex flex-col items-center justify-between gap-3 px-4 py-4 sm:flex-row sm:px-5">
      ...
      <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-start sm:gap-1">
        <button
          ...
          aria-label="이전 페이지"
          className="btn-secondary h-10 flex-1 px-3 text-[13px] fine:h-8 sm:flex-none sm:px-2.5"
        >
          <ChevronLeft size={14} />
          ...
          {getVisiblePages().map((page, index) => (
            page === '...' ? (
              ...
              <button
                ...
                className={`grid h-10 w-10 place-items-center rounded-lg text-[13px] font-medium tabular-nums transition-colors disabled:cursor-not-allowed disabled:opacity-50 fine:h-8 fine:w-8 ${
                  page === currentPage
